 ...
 * @note    上电常灭（初始高电平，亮=置低）；实时点亮由欠压检测 ISR 在欠压
 *          成立沿直控（Led_SetOn），本线程 1s 轮询欠压状态做兜底同步。
 *          独立线程（C 模式），优先级取低值，栈 1024。
 */
#include "led_task.h"
#include "hc32_drv_gpio.h"
#include "Dev/dev_power/dev_bus_voltage.h"   /* BusVoltage_IsUnderVoltage：轮询兜底 */
#include <rtthread.h>
...
static void led_thread_entry(void *param)
{
    (void)param;
    while (1) {
        /* 轮询兜底：跟随欠压状态（实时点亮在欠压 ISR 沿，此处 1s 同步一次防漏） */
        Led_SetOn(BusVoltage_IsUnderVoltage());
        Task_Set_Beat();
