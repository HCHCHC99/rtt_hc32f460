 ...
 * @note    - 慢块 A：g_volt_cfg / g_cur_cfg 配置 + 软限位校准窗口 + 停止裕量，改动少，
...
#define CUR_OVER_CUR_TH_MA_DFT      (5000.0f) /* 过流阈值 mA */
#define CUR_OVER_WINDOW_MS_DFT      (50U)     /* 过流判定窗口 ms */

/* 软限位校准窗口默认：a=b=2 / c=d=998（窄窗形式，语义见 dev_rod_position.h） */
#define ROD_CALIB_WIN_A_DFT         (2.0f)    /* 下限窗口下界 a */
#define ROD_CALIB_WIN_B_DFT         (2.0f)    /* 下限窗口上界 b */
#define ROD_CALIB_WIN_C_DFT         (998.0f)  /* 上限窗口下界 c */
#define ROD_CALIB_WIN_D_DFT         (998.0f)  /* 上限窗口上界 d */
#define ROD_STOP_MARGIN_DFT         (4.0f)    /* 推杆停止裕量 mm（位置停车：pos>=行程-裕量） */
