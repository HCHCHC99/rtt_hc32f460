 ...
 * @note    启用开关 DEV_ENABLE_MOTOR_GPIO(dev_config.h)，与 DEV_ENABLE_PWM 互斥
 *          （dev_registry.c 按 #if/#elif 二选一注册，双开时 GPIO 优先）。
 *          真值表（高电平有效，两脚成对写 + 关中断同步翻转，永不双高）：
 *          停转/急停：FWD=0 REV=0（双低 = 刹车态，待实测确认）
 *          正转(伸出)：FWD=1 REV=0      反转(缩回)：FWD=0 REV=1
 *          仲裁接缝：Init 里 Arb_BindOutputOps 绑定 fwd/rev/stop——上层链
 *          （仲裁/rod 状态机/sys_sm/过流软限位）零改动；旧板回退切 DEV_ENABLE_MOTOR_GPIO=0。
